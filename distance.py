...
# Calculate price change over the given timeframe in percentages
...
# Calculate distance using EMA *** CHECK *** Must calculated this over a timeframe, not a number of prices!
def distance_ema(active_order, prices):
...
def distance_hybrid(active_order, prices):
...
def distance_wave(active_order, price_distance):
...
def calculate(active_order, prices):
    ...
    if active_order['wiggle'] == "EMA":
        active_order = distance_ema(active_order, prices)
    ...
    if active_order['wiggle'] == "Wave":
        active_order = distance_wave(active_order, price_distance)

    ''' Use HYBRID to set distance '''
    if active_order['wiggle'] == "Hybrid":
        active_order = distance_hybrid(active_order, prices)

    # Output to stdout
    if previous_fluctuation != active_order['fluctuation']:
        defs.announce(f"Calculated trigger price distance changed to {active_order['fluctuation']:.4f} %")
